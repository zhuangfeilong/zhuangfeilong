var ZhuangFeiLong = {
	...
	parseJson: function() {
		...
		json = '[1,2,3,4,"sldkf",true,false,null]'
		...
		function parseString() {
			var startIndex = i
			var endIndex = json.slice('"',startIndex + 1)
		...
		function isDigit(char) {
			...
			var codeOf9 = '0'.charCodeAt(0)

			var charCode = char.charCodeAt(0)
			if(charCode >= codeOf0 && charCode <= codeOf0) {
				return true
			}
			else {
				return false
	...
	最大公约数: function() {
		...
		while (( m % lesser != 0 ) || (n % lesser != 0)) {
			lesser = lesser - 1
		}
		var bigger = (m * n) / (lesser)  
			console.log(lesser)
			console.log(bigger)
		},
		1-n的和: function() {
		...
		1-n以内的完全数: function() {
			...
			for (var i = first; i <= n-1; i++) {
				...
				if (Number == n) {
					console.log(n)
					console.log('yes,it is a Perfect number')
				}
				else {
					// console.log(n)
					// console.log('no')
